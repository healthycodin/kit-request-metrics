...
describe('MongoDB Storage', () => {
	...
	describe('setup()', () => {
		it('creates time series collection', async () => {
			...
			const collections = await db.listCollections({ name: 'setup_test' }).toArray();
		...
		it('accepts lazy client factory', async () => {
			...
			const collections = await db.listCollections({ name: 'factory_test' }).toArray();
			expect(collections).toHaveLength(1);
		});
	});
	...
	describe('write() and writeMany()', () => {
		...
		it('writes multiple metrics in batch', async () => {
			...
			const metrics: RequestMetric[] = [
				{ timestamp: new Date(), route: '/a', method: 'GET', status: 200, durationMs: 10 },
				{ timestamp: new Date(), route: '/b', method: 'POST', status: 201, durationMs: 20 },
				{ timestamp: new Date(), route: '/c', method: 'PUT', status: 200, durationMs: 30 }
			];
